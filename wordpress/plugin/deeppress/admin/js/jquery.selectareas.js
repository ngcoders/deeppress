...
(function($) {
        ...
        var selectValues = ['person', 'book', 'laptop', 'object', 'chair'];
    $.imageArea = function(parent, id) {
        var options = parent.options,
            ...
            updateObjectClass = function (visible) {
                if ($objectClass) {
                    $objectClass.css({
                        display: !visible ? "block" : "none",
                        left: area.x +  1,
                    ...
                    $objectClass.html($selectBox.children().filter(':input').val());
                }
                if ($selectBox) {
                    $selectBox.css({
                        display: visible ? "block" : "none",
                    ...
                    $selectBox.children().filter(':input').val(area.class);
                }
            },
            ...
            changeObjectClass = function (event) {
                cancelEvent(event);
                area.class = $(this).val();
                //var select = $('<select>').append('<option>No</option>');
                //$objectClass.append(select);
                fireEvent("changed");
            },
        ...
        var bindToChange = function ($obj) {
                $obj.click(changeObjectClass)
                    .bind("touchstart", changeObjectClass)
                    .bind("tap", changeObjectClass);
                return $obj;
            };

        $objectClass = $('<p class=\"object-class\" >').append(area.class).insertAfter($btDelete);
