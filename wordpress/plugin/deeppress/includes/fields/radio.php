...
class dp_field_radio extends dp_field
{
	/*
	*  __construct
	*
	...
	*/
	...
	function create_field( $field )
	{
		// vars
		$i = 0;
		$e = '<ul class="ba-radio-list ' . esc_attr($field['class']) . ' ' . esc_attr($field['layout']) . '">';
		...
		if( $field['other_choice'] )
		{
			...
			$e .= '<li><label><input id="' . esc_attr($field['id']) . '-other" type="radio" name="' . esc_attr($field['name']) . '" value="other" ' . $atts . ' />' . __("Other", 'ba') . '</label> <input type="text" ' . $atts2 . ' /></li>';
		}
	...
	function create_options( $field )
	{
		...
		<tr class="field_option field_option_<?php echo $this->name; ?>">
			<td class="label">
				...
				<p class="description"><?php _e("Enter your choices one per line",'ba'); ?><br />
				<br />
				<?php _e("Red",'ba'); ?><br />
				<?php _e("Blue",'ba'); ?><br />
				<br />
				<?php _e("red : Red",'ba'); ?><br />
				<?php _e("blue : Blue",'ba'); ?><br />
				</p>
			</td>
			<td>
				<?php
				...
				do_action('dp/create_field', array(
					...
					'message'	=>	__("Add 'other' choice to allow for custom values", 'ba')
				));
				...
				do_action('dp/create_field', array(
					...
					'message'	=>	__("Save 'other' values to the field's choices", 'ba')
				));
		...
		<tr class="field_option field_option_<?php echo $this->name; ?>">
			<td class="label">
				<label for=""><?php _e("Layout",'ba'); ?></label>
			</td>
			<td>
				<?php
				...
				do_action('dp/create_field', array(
					...
					'layout' => 'horizontal', 
					'choices' => array(
						'vertical' => __("Vertical",'ba'),
						'horizontal' => __("Horizontal",'ba')
					)
				));
	...
	function update_value( $value, $post_id, $field )
	{
		...
		if( $field['save_other_choice'] )
		{
			...
			if( !isset($field['choices'][ $value ]) )
			{
				...
				{
					do_action('dp/update_field', $field, $field['field_group']);
				}
