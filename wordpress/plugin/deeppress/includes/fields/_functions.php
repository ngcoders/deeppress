...
class dp_field_functions
{
	...
	*/
	...
	{
		//value
		add_filter('dp/load_value', array($this, 'load_value'), 5, 3);
		add_action('dp/update_value', array($this, 'update_value'), 5, 3);
		add_action('dp/delete_value', array($this, 'delete_value'), 5, 2);
		add_action('dp/format_value', array($this, 'format_value'), 5, 3);
		add_action('dp/format_value_for_api', array($this, 'format_value_for_api'), 5, 3);


		// field
		add_filter('dp/load_field', array($this, 'load_field'), 5, 3);
		add_action('dp/update_field', array($this, 'update_field'), 5, 2);
		add_action('dp/delete_field', array($this, 'delete_field'), 5, 2);
		add_action('dp/create_field', array($this, 'create_field'), 5, 1);
		add_action('dp/create_field_options', array($this, 'create_field_options'), 5, 1);


		// extra
		add_filter('dp/load_field_defaults', array($this, 'load_field_defaults'), 5, 1);
	}
	...
	*/
	...
	{
		$found = false;
		$cache = wp_cache_get( 'load_value/post_id=' . $post_id . '/name=' . $field['name'], 'ba', false, $found );
		...
		{
			// run filters
			$value = apply_filters('dp/load_value/' . $key . '=' . $field[ $key ], $value, $post_id, $field); // new filter
		}


		//update cache
		wp_cache_set( 'load_value/post_id=' . $post_id . '/name=' . $field['name'], $value, 'ba' );
	...
	*/
	...
	{
		$value = apply_filters('dp/format_value/type=' . $field['type'], $value, $post_id, $field);
	...
	*/
	...
	{
		$value = apply_filters('dp/format_value_for_api/type=' . $field['type'], $value, $post_id, $field);
	...
	*/
	...
	function update_value( $value, $post_id, $field )
	{
		...
		{
			// run filters
			$value = apply_filters('dp/update_value/' . $key . '=' . $field[ $key ], $value, $post_id, $field); // new filter
		}
	...
	*/
	...
	function delete_value( $post_id, $key )
	{
		...
		wp_cache_delete( 'load_value/post_id=' . $post_id . '/name=' . $key, 'ba' );
	}
	...
	*/
	...
	function load_field( $field, $field_key, $post_id = false )
	{
		...
		if( !$field )
		{
			$field = wp_cache_get( 'load_field/key=' . $field_key, 'ba' );
		}
		...
		if( !$field )
		{
			...
			if( !empty($rows) )
			{
				...
				*/
				...
				{
					$wpml_post_id = icl_object_id($row['post_id'], 'ba', true, ICL_LANGUAGE_CODE);
		...
		// apply filters
		$field = apply_filters('dp/load_field_defaults', $field);
		...
		{
			// run filters
			$field = apply_filters('dp/load_field/' . $key . '=' . $field[ $key ], $field); // new filter
		}


		// set cache
		wp_cache_set( 'load_field/key=' . $field_key, $field, 'ba' );
	...
	*/
	...
	function load_field_defaults( $field )
	{
		...
		// Parse Values
		$field = apply_filters( 'dp/parse_types', $field );


		// field specific defaults
		$field = apply_filters('dp/load_field_defaults/type=' . $field['type'] , $field);
	...
	*/
	...
	function update_field( $field, $post_id )
	{
		...
		// filters
		$field = apply_filters('dp/update_field/type=' . $field['type'], $field, $post_id ); // new filter


		// clear cache
		wp_cache_delete( 'load_field/key=' . $field['key'], 'ba' );
	...
	*/
	...
	{
		// clear cache
		wp_cache_delete( 'load_field/key=' . $field_key, 'ba' );
	...
	*/
	...
	function create_field( $field )
	{
		...
		do_action('dp/create_field/type=' . $field['type'], $field);
		...
		if( $field['conditional_logic']['status'] )
		{
			...
			<script type="text/javascript">
                (function($) {
                    ...
                    {
                        ba.conditional_logic.items.push(<?php echo json_encode($field['conditional_logic']); ?>);
                    }
	...
	*/
	...
	function create_field_options($field)
	{
		...
		// render HTML
		do_action('dp/create_field_options/type=' . $field['type'], $field);
	}
